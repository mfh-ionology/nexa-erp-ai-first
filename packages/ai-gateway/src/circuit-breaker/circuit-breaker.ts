...
/**
 * Circuit breaker for the Platform API quota check.
 *
 * State transitions:
 ...
 *
 * When OPEN, the circuit breaker prevents calls and returns a degraded result.
 */
export class CircuitBreaker {
   ...
   *
   * @param fn The function to execute (e.g., the HTTP call to Platform API)
  ...
  async execute<T>(fn: () => Promise<T>, fallbackFn?: () => T): Promise<T> {
    ...
    if (this.state === 'OPEN') {
      ...
      if (elapsed >= this.recoveryWindowMs && !this.halfOpenProbeInFlight) {
        // ISSUE #6 FIX: Set halfOpenProbeInFlight BEFORE transitioning to prevent
    ...
    // Guard concurrent probes in HALF_OPEN state (for probes that entered via CLOSED → HALF_OPEN elsewhere)
    ...
    try {
      ...
      // ISSUE #25 FIX: Log probe failure before returning fallback
