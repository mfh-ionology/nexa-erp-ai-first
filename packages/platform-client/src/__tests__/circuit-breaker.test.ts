...
describe('CircuitBreaker', () => {
  ...
  describe('HALF_OPEN state', () => {
    ...
    it('returns fallback on HALF_OPEN probe failure when fallback provided', async () => {
      const fn = vi.fn().mockRejectedValue(new Error('still down'));
      const fallback = vi.fn().mockReturnValue('stale-cache');
  ...
  // ─── 4xx errors skip circuit breaker ──────────────────────────────────

  describe('__skipCircuitBreaker for 4xx client errors', () => {
    it('does not count failures with __skipCircuitBreaker flag', async () => {
      ...
      // Should NOT have opened — 4xx errors are not infrastructure failures
      expect(breaker.getState()).toBe('CLOSED');
      expect(breaker.getFailureCount()).toBe(0);
    });

    it('still throws the error even when skipping failure tracking', async () => {
      const clientError = Object.assign(new Error('Not Found'), {
        __skipCircuitBreaker: true,
      });
      const fn = vi.fn().mockRejectedValue(clientError);

      await expect(breaker.execute(fn)).rejects.toThrow('Not Found');
    });

    it('tracks failures for errors without __skipCircuitBreaker', async () => {
    ...
    it('mixes skippable and non-skippable errors correctly', async () => {
      const clientError = Object.assign(new Error('Bad Request'), {
        __skipCircuitBreaker: true,
      });
      ...
      // 4xx error — skipped
      await expect(breaker.execute(vi.fn().mockRejectedValue(clientError))).rejects.toThrow();
      expect(breaker.getFailureCount()).toBe(0);

      // 5xx error — counted
      await expect(breaker.execute(vi.fn().mockRejectedValue(serverError))).rejects.toThrow();
      expect(breaker.getFailureCount()).toBe(1);
      ...
      // 5xx error — counted (now 2)
      await expect(breaker.execute(vi.fn().mockRejectedValue(serverError))).rejects.toThrow();
      expect(breaker.getFailureCount()).toBe(2);

      // Still CLOSED — only 2 real failures (threshold is 3)
      expect(breaker.getState()).toBe('CLOSED');
    });
  });
