...
**Key Tasks:**
- [x] Create Prisma models for all 6 tables with proper relations and indexes
- [x] Create migration and seed script (system variables, example automation)
- [x] Implement automation scheduler service (cron-based, timezone-aware)
- [x] Implement event-triggered automation service (listens to event bus)
- [x] Implement step execution engine (sequential, with input/output piping)
- [x] Implement goal-oriented agent executor (multi-turn autonomous execution)
- [x] Implement automation chaining (trigger next automation on completion)
- [x] Implement circular chain detection (graph traversal, max depth 10, reject cycles)
- [x] Implement prompt variable resolver (DB_FIELD, DB_QUERY, SYSTEM, PREVIOUS_STEP, CONSTANT, EXPRESSION)
- [x] Implement automation CRUD API endpoints
- [x] Implement automation run/step-run query endpoints (with pagination, filtering)
- [x] Add circuit breaker: if automation fails 3 consecutive times, auto-pause and notify admin
- [x] Implement mandatory parameter validation in orchestrator: validate all `required` fields (top-level and nested) from tool `inputSchema` BEFORE executing any tool call
- [x] Implement clarification prompt generation: when required params are missing, build a user-facing prompt listing all missing fields with descriptions (batch — not one-at-a-time)
- [x] Implement nested schema validation: traverse array item schemas to check nested `required` fields (e.g., `lines[].quantity`, `conditions[].operator`)
- [x] Implement AUTONOMOUS mode param failure: when variable binding cannot resolve a required param, fail step with `UNRESOLVABLE_REQUIRED_PARAM` error
...
**Key Tasks:**
- [x] Implement variable parser (extract `{{varName}}` from template text)
- [x] Implement variable resolver with pluggable source handlers (DB_FIELD, DB_QUERY, PAGE_FIELD, SYSTEM, PREVIOUS_STEP, CONSTANT, EXPRESSION)
- [x] Implement DB field resolver with companyId scoping and relation traversal (e.g., `customer.primaryContact.email`)
- [x] Implement expression evaluator (date arithmetic, string operations, basic math)
- [x] Implement system variable provider (today, currentUser, company, etc.)
- [x] Implement variable registry API (GET /ai/variables — list all available variables with types and descriptions)
- [x] Implement graceful fallback for unresolvable variables
- [x] Add comprehensive test coverage for each variable source type
...
| E5c.3 | AI Model & Prompt Admin UI | backlog |
| E5c.4 | Agent & Skill Admin UI | backlog |
| E5c.5 | Automation Builder UI | backlog |
| E5c.6 | Automation Monitoring & Run History | backlog |
