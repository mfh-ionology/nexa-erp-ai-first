...
## Story E9.1: Notification Service
...
**Key Tasks:**
- [x] Implement notification creation from event bus handlers (AC: #1)
  - [x] Subscribe to business events via NotificationTemplate.eventName matching
  - [x] Resolve target users from template rules (entity owner, role-based, specific users)
  - [x] Create Notification records with PENDING status
- [x] Implement template rendering engine (AC: #2)
  - [x] Handlebars-based variable substitution in title and body
  - [x] Support for entity data, user data, and computed values
- [x] Implement delivery orchestrator (AC: #3, #4, #5)
  - [x] Read user NotificationPreference for each event type and channel
  - [x] Cascade: user preference -> template defaults (BR-COM-014)
  - [x] Dispatch to each channel independently via BullMQ jobs
  - [x] Update Notification.status per channel (DELIVERED, FAILED)
- [x] Implement NotificationTemplate and NotificationPreference CRUD (AC: #1, #4)
  - [x] Seed default templates for common events (approval.requested, invoice.approved, etc.)
...
## Story E9.2: In-App Notifications
...
## Story E9.3: Email Notification Channel
...
## Story E9.4: Notification Preferences
...
| Story | Title | Status |
|-------|-------|--------|
| E9.1 | Notification Service | done |
| E9.2 | In-App Notifications | backlog |
| E9.3 | Email Notification Channel | backlog |
| E9.4 | Notification Preferences | backlog |
