...
- `apps/web/src/components/templates/data-table.tsx` — Refined skeleton loading widths, replaced ArrowUpDown with ChevronsUpDown sort icon, polished empty state with rounded icon container and hint text, added group hover for sort indicator
- `apps/web/src/components/ui/table.tsx` — Wrapped table in rounded border container with `bg-card`, added `bg-muted/40` header background, changed header text to uppercase/tracking-wider/muted, updated row hover to `hover:bg-accent/50`, updated cell padding to `px-4 py-3.5`
- `apps/web/src/components/layout/app-sidebar.tsx` — Added `min-h-0` to ScrollArea for flex overflow fix
- `apps/web/src/features/admin/users/user-badge-styles.ts` — Aligned role/status badge colors to Concept D hex tokens (removed dark mode variants)
- `apps/web/src/features/admin/users/user-list-page.tsx` — Added avatar circles to name column (merged email below name), replaced status badge with dot indicator, access group count in circle badge
...
- `apps/web/src/routes/403.tsx` — Card wrapper, ShieldX icon, Button component, staggered animations; consolidated i18n to `errors:` namespace
- `packages/i18n/locales/en/common.json` — Added 80+ translation keys for dashboard, modules; removed dead `dashboard.dateSubtitle` key; added `noResultsHint` key; removed `errors.*` flat keys (moved to errors namespace)
- `packages/i18n/locales/en/errors.json` — Added `accessDeniedTitle` and `backToDashboard` keys (consolidated from common.json)
- `package.json` — Added `shadcn` dev dependency

## Senior Developer Review (AI)

**Reviewer:** Mohammed (via Claude Opus 4.6 adversarial review)
**Date:** 2026-02-24
...
**HIGH — Fixed:**
- **H1** Dashboard date hardcoded to `en-GB` locale (AC #8 violation) → Fixed: uses `useLocale()` from `@nexa/i18n`
- **H2** `dashboard.dateSubtitle` i18n key defined but never used → Fixed: removed dead key from common.json
- **H3** 7 application files changed but not in story File List → Fixed: File List updated from 17 to 24 modified files

**MEDIUM — Fixed:**
- **M4** Mixed i18n namespace pattern in 403 page (`errors.` flat keys vs `errors:` namespace) → Fixed: consolidated all 403 keys to `errors:` namespace; moved `accessDeniedTitle` and `backToDashboard` to errors.json
- **M5** `noResultsHint` i18n key referenced in data-table.tsx but missing from common.json → Fixed: added key; removed `|| ''` fallback
- **M6** `table.tsx` TableHead styling is a global-impact undocumented change (uppercase/tracking-wider/muted headers) → Fixed: documented in File List
- **M7** `table.tsx` TableCell removes `whitespace-nowrap` (potential wrapping regression) → Fixed: documented in File List; intentional for visual polish
- **M8** `STATUS_BADGE_STYLES` initially flagged as dead code → Downgraded: still used by `user-detail-page.tsx`

**LOW — Accepted (no fix needed):**
- **L9** Sparkline SVG colors are hardcoded hex values — accepted; SVG stroke doesn't support Tailwind classes
- **L10** Repeated `Math.max(...data)` in chart helpers — non-issue for 12-element arrays
- **L11** `.mcp.json` untracked in repo root — tooling config, separate from story scope

### Review Fix Summary
- `routes/_authenticated/index.tsx` — replaced hardcoded `'en-GB'` locale with `useLocale()` from `@nexa/i18n`
- `routes/403.tsx` — changed `t('errors.accessDeniedTitle')` → `t('errors:accessDeniedTitle')` and `t('errors.backToDashboard')` → `t('errors:backToDashboard')`
- `packages/i18n/locales/en/common.json` — removed `dashboard.dateSubtitle`, `errors.accessDeniedTitle`, `errors.backToDashboard`; added `noResultsHint`
- `packages/i18n/locales/en/errors.json` — added `accessDeniedTitle`, `backToDashboard`
- `data-table.tsx` — removed `|| ''` fallback on `noResultsHint`
- Story File List updated to include all 7 previously undocumented files

### Second-Pass Review

**Reviewer:** Mohammed (via Claude Opus 4.6 adversarial review, pass 2)
**Date:** 2026-02-24
**Outcome:** Changes Requested → Fixed

First-pass fix verification: all 8 fixes correctly applied, typecheck clean, no regressions.

**Issues Found (4 total: 1 Medium, 3 Low) — All Fixed:**
- **M1** Missing `showingRecords` i18n key in `entity-list-page.tsx:312` — `t('showingRecords')` returns literal key name (i18next `returnNull: false`), `||` fallback is dead code → Fixed: replaced with existing `t('recordCount', { count })` pluralization key
- **L2** Dashboard task checkboxes lack `onCheckedChange` handler — appear interactive but frozen → Fixed: added `disabled` prop
- **L3** Bar chart container missing `aria-hidden="true"` — inconsistent with line chart and sparklines → Fixed: added `aria-hidden="true"`
- **L4** Cash flow SVG hardcodes `#7c3aed`/`#e5e7eb` instead of CSS custom properties → Fixed: uses `var(--color-primary)` and `var(--color-border)`

**Additional fix:** `common.json` had trailing comma + blank line before closing brace — invalid JSON that passed typecheck but broke Vite build → Fixed: removed trailing comma and blank line

### Second-Pass Fix Summary
- `entity-list-page.tsx` — replaced `t('showingRecords', ...)` with `t('recordCount', { count })`, removed dead `||` fallback
- `routes/_authenticated/index.tsx` — added `disabled` to task checkboxes, added `aria-hidden="true"` to bar chart, replaced hardcoded SVG hex colors with CSS custom properties
- `packages/i18n/locales/en/common.json` — fixed trailing comma + blank line (invalid JSON)
