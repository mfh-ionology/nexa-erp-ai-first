...
## Story E5d.S1: Knowledge Base Schema & RAG Pipeline
...
**Key Tasks:**
- [ ] Create Prisma models for 5 tenant-side tables with vector columns and GIN indexes
- [ ] Create migration and seed script (default knowledge categories)
- [ ] Implement document upload endpoint (text, markdown, PDF support)
- [ ] Implement document chunking service (recursive text splitter, ~500 tokens per chunk, overlap 50 tokens)
- [ ] Reuse `EmbeddingService` from E5b.S4 for chunk embedding generation (do NOT create separate)
- [ ] Implement RAG retrieval service using E5b's `VectorSearchService.similaritySearch()` with confidence-weighted re-ranking (do NOT create separate vector search)
- [ ] Implement knowledge injection into dynamic context assembly (integrate with E5b's context assembler)
- [ ] Implement knowledge CRUD endpoints (POST, GET, PATCH, DELETE)
- [ ] Implement knowledge article categories (BUSINESS_PROCESS, TERMINOLOGY, INDUSTRY_RULES, CUSTOM_FIELDS, HISTORICAL_PATTERN)
...
## Story E5d.S2: Correction Loop & Training Examples
...
**Key Tasks:**
- [ ] Implement correction capture middleware (intercepts user corrections from chat, feedback, and edit actions)
- [ ] Implement correction categorisation (auto-categorise: TERMINOLOGY, PROCESS, DATA, PREFERENCE, OTHER)
- [ ] Implement pattern detection service (detect N corrections on same topic → draft knowledge article)
- [ ] Implement training example CRUD endpoints and few-shot injection
- [ ] Implement learning signals aggregation service (daily cron, per-skill metrics)
- [ ] Implement correction review API (admin: list, group, confirm/reject auto-generated knowledge)
- [ ] Implement high-correction-rate alert (threshold: 30% → flag skill for review)
- [ ] Add comprehensive test coverage for correction loop scenarios

**FR/NFR:** FR4; NFR2

---

## Story E5d.S3: Cross-Tenant Intelligence Pipeline (Platform)
...
**Key Tasks:**
- [ ] Create Platform Prisma models for 5 tables
- [ ] Create migration and seed script
- [ ] Implement anonymisation service (strip all PII, verify with automated PII detection)
- [ ] Implement daily aggregation job (tenant → platform, respecting opt-out)
- [ ] Implement skill effectiveness aggregation (cross-tenant skill metrics)
- [ ] Implement weekly insights generation service (feature gaps, workflow opportunities, default optimisation)
- [ ] Implement pattern query API for Platform Admin Portal (with filters: industry, plan tier, tenant size)
- [ ] Implement opt-out mechanism (tenant setting, verified data exclusion)
- [ ] Add comprehensive test coverage including PII verification tests

**FR/NFR:** FR6; NFR50 (data privacy)

---

## Story E5d.S4: Platform Knowledge Distribution
...
## Story E5d.S5: Knowledge Management UI (Tenant)
...
## Story E5d.S6: Platform Intelligence Dashboard (Vendor)
