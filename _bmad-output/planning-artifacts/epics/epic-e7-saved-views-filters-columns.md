...
## Story E7.S1: Database Schema & API Foundation
...
## Story E7.S2: Column Customization & Views UI
...
## Story E7.S3: Filter Builder & Favourites
...
8. GIVEN a filter configuration WHEN included in a saved view (via E7.S2) THEN the filters persist and re-apply when the view is loaded
...
## Story E7.S4: Toolbar Redesign — Split Buttons, Views Bar & Quick Filter

**User Story:** As a user, I want a cleaner toolbar with separate Columns popover, Quick Filter modal, and Advanced Filter modal, plus a views bar showing all my saved views as pill tabs, so that I can switch views instantly and access filters without navigating combined modals.

**Design Document:** `docs/plans/2026-03-01-toolbar-redesign-design.md` (approved)

**Acceptance Criteria:**

1. GIVEN any T1 Entity List page WHEN rendered on desktop/tablet THEN the toolbar shows two rows: Row 1 with [Search] [Columns] [Filter] [Advanced] and Row 2 with pill tabs for saved views plus [Save] [Delete] buttons
2. GIVEN the [Columns] button (Columns3 icon) WHEN clicked THEN a popover (~320px) drops below showing all columns with drag-reorder handles, visibility checkboxes, and pin cycle buttons (NONE → LEFT → RIGHT → NONE), with [Reset to Default] and [Apply] footer buttons
3. GIVEN the [Filter] button (Filter/funnel icon) WHEN clicked THEN a modal opens showing metadata-driven filter fields loaded from `data_view_fields` where `filterable=true` AND `advancedFilterOnly=false`, with controls driven by `fieldType` (ENUM → multi-select from LOV, BOOLEAN → three-state, DATE → preset picker, STRING → text input, NUMBER → range), footer has [Reset] [Apply]
4. GIVEN the [Advanced] button (SlidersHorizontal icon) WHEN clicked THEN a wider modal (~640px) opens with two tabs: Filters (condition builder with AND/OR groups, all operators, all fields including advancedFilterOnly=true) and Sort (drag-reorderable priority rules, max 5)
5. GIVEN filter state WHEN filters are applied from either Quick Filter or Advanced Filter THEN both buttons show the same active filter count badge, and the filter state is shared between them
6. GIVEN saved views exist WHEN rendered on desktop/tablet THEN they appear as horizontal pill tabs in Row 2, with "All" always first, active pill in `bg-primary text-primary-foreground`, favourites showing a ★ indicator, ordered by favourites first then alphabetical
7. GIVEN saved views exist WHEN rendered on phone THEN the pill tabs are replaced by a dropdown selector showing the active view name with a chevron
8. GIVEN more views than fit in Row 2 WHEN the views overflow THEN a "+N more" pill appears that opens a dropdown with the remaining views
9. GIVEN the [Save] button WHEN clicked with a dirty named view loaded THEN a confirm dialog asks "Replace view '{name}'?" with [Replace] [Save as New] [Cancel] options
10. GIVEN the [Save] button WHEN clicked with no named view or a clean named view THEN a "Save as New" popover form appears with name input, "Add to favourites" checkbox, and scope selector (Personal/Team/Global)
11. GIVEN the [Delete] button WHEN a named view is loaded THEN it is enabled; clicking shows a confirm dialog; on confirm the view is deleted and "All" is activated
12. GIVEN the [Delete] button WHEN "All" (default) is active THEN the button is disabled

**Key Tasks:**
- [ ] Create `<ColumnsButton>` + `<ColumnsPopover>` — popover with drag-reorder, visibility, pin (reuse ColumnsTab internals)
- [ ] Create `<QuickFilterButton>` + `<QuickFilterModal>` — metadata-driven simple filter (reuse SimpleFilterPanel)
- [ ] Create `<AdvancedFilterButton>` + `<AdvancedFilterModal>` — condition builder + sort (reuse AdvancedFilterPanel + SortTab)
- [ ] Create `<ViewsBar>` — pill tabs for desktop/tablet, dropdown for phone (responsive swap)
- [ ] Create `<SaveViewButton>` with smart save logic (replace vs save-as-new popover)
- [ ] Create `<DeleteViewButton>` with confirm dialog
- [ ] Update `entity-list-page.tsx` to use new toolbar layout (two rows) and remove old components
- [ ] Remove old components: `ViewsColumnsButton`, `FilterSortButton`, `ViewsAndColumnsModal`, `FilterSortModal`
- [ ] Update `SavedViewSelector` → keep for phone-only use inside ViewsBar
- [ ] Ensure shared filter state between Quick Filter and Advanced Filter via `useFilterState`
- [ ] Ensure all components match Concept D visual design (purple theme, 12px radius, custom shadows)
- [ ] Add unit tests for all new components
- [ ] Update existing tests that reference removed components

**FR/NFR:** FR86; NFR27 (WCAG 2.1 AA), NFR28

**Reference Documents:**
| Document | Section | Key Items |
|----------|---------|-----------|
| Architecture | §5.2.1 Standardised Page Templates | T1 Entity List toolbar pattern |
| API Contracts | §2.3 Views, §3.13 | All view/column/filter endpoints (no API changes) |
| Data Models | §3.1 System Module | SavedView, UserColumnPreference, DataViewField |
| UX Design Spec | §T1 Entity List Template | Toolbar buttons, saved view selector, column/filter UI |
| Project Context | §13 Metadata-Driven DataTable | Auto-generated columns, LOV strategy |
| Design Doc | `docs/plans/2026-03-01-toolbar-redesign-design.md` | Full approved design with layout, components, behaviour |
